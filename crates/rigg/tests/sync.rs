...
#[tokio::test]
async fn push_dry_run_reports_a_leaked_binding_without_refusing() {
    let server = MockServer::start().await;
    mock_empty_lists(&server).await;
    Mock::given(method("GET"))
        .and(path("/datasources/docs"))
    ...
    assert!(
        mutations.is_empty(),
        "a dry run must never mutate, saw {mutations:?}"
    );
}
...
#[tokio::test]
async fn push_relink_preflight_refuses_a_leaked_binding_before_relinking() {
    let server = MockServer::start().await;
    mock_empty_lists(&server).await;
    Mock::given(method("GET"))
        .and(path("/knowledgeSources/test-ks"))
    ...
    // instead of the ordinary plan.
    let tmp = tempfile::tempdir().unwrap();
    std::fs::write(
        tmp.path().join("rigg.yaml"),
        format!(
            "environments:\n  dev:\n    default: true\n    search: {{ service: mock, endpoint: \"{e}\" }}\n    dependencies:\n      aoai: {{ ai-services: devaisvc }}\n  prod:\n    policy: {{ protected: true }}\n    search: {{ service: mock, endpoint: \"{e}\" }}\n",
            e = server.uri()
        ),
    )
    .unwrap();
    ...
    let state_dir = tmp.path().join(".rigg/prod/demo");
    std::fs::create_dir_all(&state_dir).unwrap();
    std::fs::write(
        state_dir.join("replace-test-ks.json"),
        serde_json::to_string_pretty(&json!({
            ...
            "knowledge_bases": [{
                ...
                "models": [{
                    ...
                    "azureOpenAIParameters": {
                        "resourceUri": "https://devaisvc.openai.azure.com",
                        "deploymentId": "gpt"
                    }
                }]
            }]
    ...
    rigg(tmp.path())
        ...
        .code(3)
        .stderr(
            predicate::str::contains("model host 'devaisvc'")
                .and(predicate::str::contains(
                    "bound in environment 'dev' as 'aoai'",
                ))
                .and(predicate::str::contains("nothing was pushed")),
        );
    ...
    assert!(
        mutations.is_empty(),
        "the relink preflight must refuse before any mutation, saw {mutations:?}"
    );
    assert!(
        state_dir.join("replace-test-ks.json").exists(),
        "recovery file must survive a refused relink"
    );
}
