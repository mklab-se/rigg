...
/// Local state tracking (.rigg/<env>/state.json)
...
/// Checksums for change detection (.rigg/<env>/checksums.json)
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
impl LocalState {
    ...
    pub const DIR_NAME: &'static str = ".rigg";
    /// State file name
...
mod tests {
    ...
    fn test_env_dir_path() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/my/project/.rigg/prod")
        );
    }
    ...
    fn test_state_file_env_path() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/my/project/.rigg/prod/state.json")
        );
    }
    ...
    fn test_checksums_file_env_path() {
        ...
        assert_eq!(
            ...
            PathBuf::from("/my/project/.rigg/test/checksums.json")
        );
    }
    ...
    fn test_checksums_load_bare_empty_json() {
        let dir = tempfile::tempdir().unwrap();
        let env_dir = dir.path().join(".rigg").join("test");
