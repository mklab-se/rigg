//! MCP tool implementations for hoist
...
/// The hoist MCP server
#[derive(Clone)]
pub struct HoistMcpServer {
...
#[tool_router]
impl HoistMcpServer {
    /// Show hoist project status including environment, auth, and resource counts.
    #[tool(
        description = "Show hoist project status: environment info, auth state, resource counts, and last sync time"
    )]
    async fn hoist_status(&self, Parameters(params): Parameters<EnvParam>) -> String {
    ...
    async fn hoist_describe(&self, Parameters(params): Parameters<EnvParam>) -> String {
    ...
    #[tool(description = "List all configured deployment environments from hoist.yaml")]
    async fn hoist_env_list(&self) -> String {
    ...
    async fn hoist_validate(&self, Parameters(params): Parameters<ValidateParams>) -> String {
    ...
    async fn hoist_list(&self, Parameters(params): Parameters<ListParams>) -> String {
        match build_list(
            params.env.as_deref(),
...
#[tool_handler]
impl ServerHandler for HoistMcpServer {
...
fn build_status(env_override: Option<&str>) -> anyhow::Result<serde_json::Value> {
    ...
    let state = hoist_core::state::LocalState::load_env(&project_root, &env.name)?;
    ...
    if source == "remote" || source == "both" {
        ...
        if !search_kinds.is_empty() {
            if let Some(search_svc) = env.primary_search_service() {
                let client = hoist_client::AzureSearchClient::from_service_config(search_svc)?;
