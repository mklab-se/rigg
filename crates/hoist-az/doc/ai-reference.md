# hoist — AI Agent Reference

## Tool Description

hoist is a configuration-as-code CLI for **Azure AI Search** and **Microsoft Foundry**. It pulls resource definitions from Azure as normalized JSON (or YAML for agents), lets you edit them locally under Git, and pushes changes back. Think of it as "Terraform for Azure Search and Foundry agents."
...
| Command | Description |
|---------|-------------|
| `hoist init` | Initialize a new hoist project (creates `hoist.yaml`, directory structure) |
| `hoist status` | Show sync status, resource counts, environment info |
| `hoist describe` | Show a unified summary of all local resource definitions |
| `hoist validate [--strict] [--check-references]` | Validate local JSON files for structural and referential integrity |

### Sync Commands

| Command | Description |
|---------|-------------|
| `hoist pull [FLAGS]` | Download resource definitions from Azure to local JSON files |
| `hoist push [FLAGS]` | Upload local JSON files to Azure, creating or updating resources |
| `hoist diff [FLAGS]` | Compare local resource files against the live Azure service |
| `hoist pull-watch [FLAGS]` | Poll the server for changes and pull updates automatically |

### Resource Management

| Command | Description |
|---------|-------------|
| `hoist new <type> <name> [OPTIONS]` | Create a new resource file from a template (no network calls) |
| `hoist copy <source> <target> --<type>` | Copy a resource locally under a new name (no network calls) |
| `hoist delete --<type> <name> --target <remote\|local>` | Delete a resource from Azure or remove local files |

### Environment & Auth

| Command | Description |
|---------|-------------|
| `hoist env list` | List all configured environments |
| `hoist env show [name]` | Show details for an environment |
| `hoist env set-default <name>` | Set the default environment |
| `hoist env add <name>` | Add a new environment (interactive ARM discovery) |
| `hoist env remove <name>` | Remove an environment |
| `hoist auth login` | Authenticate with Azure |
| `hoist auth status` | Check current authentication status |
| `hoist auth logout` | Clear cached authentication |

### Configuration

| Command | Description |
|---------|-------------|
| `hoist config show` | Display current configuration from `hoist.yaml` |
| `hoist config set <key> <value>` | Set a configuration value |
| `hoist config init` | Interactive configuration setup |

### AI & MCP

| Command | Description |
|---------|-------------|
| `hoist ai` | Show AI feature status |
| `hoist ai enable` | Enable AI features |
| `hoist ai disable` | Disable AI features |
| `hoist ai config` | Interactive AI provider/model configuration |
| `hoist ai test [message]` | Test AI integration |
| `hoist ai skill` | Show skill setup guide |
| `hoist ai skill --emit` | Output skill markdown file to stdout |
| `hoist ai skill --reference` | Output this reference documentation |
| `hoist mcp serve` | Start MCP server (stdio transport) |
| `hoist mcp install [claude-code\|vs-code] [--scope workspace\|global]` | Register hoist as an MCP server |
...
project-root/
  ...
  .hoist/                             # Per-environment state (gitignored)
    <env>/
      state.json
      checksums.json
...
3. **Validate** structural integrity:
   ```
   hoist validate --strict --check-references
   ```
...
- Use `--recursive` with singular flags to include dependent resources (e.g., `hoist pull --knowledgesource myks --recursive`).
...
hoist env show staging

# Change default environment
hoist env set-default staging

# Add a new environment (interactive)
hoist env add staging

# Remove an environment
hoist env remove old-env
```

The `--env <name>` flag (or `HOIST_ENV` env var) on any command targets a specific environment. If omitted, the default environment is used.
...
2. **Cascade push order.** When pushing a knowledge source, hoist pushes in dependency order: Knowledge Source -> Index -> Skillset -> Data Source -> Indexer.

3. **Known Azure update bug.** Azure sometimes fails to update knowledge sources in-place. When this happens, hoist detects the failure and offers to **delete and recreate** the knowledge source (which also recreates all managed sub-resources). This is safe — the data will be re-indexed automatically.
...
## hoist.yaml Configuration Format
...
The hoist MCP server exposes 9 tools. Start it with `hoist mcp serve` or install via `hoist mcp install claude-code`.
...
All mutating tools (`hoist_pull`, `hoist_push`, `hoist_delete`) follow the same pattern:
...
1. **Always validate before push:** Run `hoist validate` (or `hoist_validate` MCP tool) before pushing to catch structural errors.
2. **Always diff before push:** Run `hoist diff` (or `hoist_diff` MCP tool) to review changes before pushing.
3. **Pull before push for conflict detection:** If you haven't pulled recently, pull first so hoist can detect if the remote has changed since your last sync.
4. **Use `--force` only in CI/CD or after previewing:** In interactive sessions, let hoist show the preview and ask for confirmation.
...
hoist delete --index my-index --target remote

# Delete local files (Azure resource is kept)
hoist delete --index my-index --target local
```
...
The `hoist new` command creates resource files from templates without making any network calls.
...
hoist new datasource my-ds --type azureblob --container documents

# Create a new indexer
hoist new indexer my-indexer --datasource my-ds --index my-index --skillset my-skillset

# Create a new skillset
hoist new skillset my-skillset

# Create a new synonym map
hoist new synonym-map my-synonyms

# Create a new alias
hoist new alias my-alias --index my-index

# Create a new knowledge base
hoist new knowledge-base my-kb
...
hoist new agentic-rag my-system --model gpt-4o --datasource-type azureBlob --container documents
```
