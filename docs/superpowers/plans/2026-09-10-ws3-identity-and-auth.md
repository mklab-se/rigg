...
Executed on branch `rigg-2`, commits dbe19a0..c6c6cda. Rulings:

- Task 1 (identity graph v2, dbe19a0): review Approved with one Important — push's 1.x `diagnose_rbac` loses the storage case. Ruling: fixed in Task 4 by routing the PUT-time diagnosis through `auth_engine::verify(Plan)`.
- Task 2 (client, bdb342a): Critical — service-principal tokens were minted with an invalid `az … --username`. Ruling: mint via OAuth2 `client_credentials` / federated `client_assertion` at a `RIGG_LOGIN_ENDPOINT`-overridable login endpoint (4e7327a); merge-not-replace for storage bypass / UAMI map / Graph appRoles; `atScope()` reads keep inherited assignments, rigg-tagged listing is at-scope-exact; nextLink paging.
- Task 3 (doctor v2, b908d50): Critical — `--fix` granted the operator their own roles. Ruling: `Report::fixes()` never includes operator/Named edges, the `az` line is always printed and exit stays 4 (d1953b5). `--principal <oid>` is report-only too. Task 2 + 3 fix rounds shared one re-review: clean.
- Task 4 (push preflight / verify, 720e7e7): Critical — role grants and storage network fixes ran before the protected-environment gate. Ruling: verification + refusal stay early; consent + apply + propagation wait move after `confirm_protected_env` and the "Apply N change(s)?" gate, before the first data-plane PUT (02b6ad6). `rigg verify` gets the protected gate (billable indexer runs) and `--confirm-env`; MCP `rigg_verify{confirm_env, answers}`. Re-review clean.
- Task 5 (Easy Auth, key sources, `--identity`, 9d34bc9): Importants — skillset `--identity` scaffold lacked the `AIServicesByIdentity` discriminator (fixed; knowledge-source dropped from the accepting set — its scaffold is `searchIndex`), `assign_app_role` not idempotent (fixed), injected keys relied on Azure redacting its echo (fixed: carriers restored from the local document before disk/baseline, proven against a verbatim-echo fake) (6127717). No Critical → no scoped re-review; covered by the final review. Ruling: the system identity is admitted to `allowedApplications` when a matching skill names no `authIdentity`.
- Task 6 (docs, ci init, live smoke, 677096d): live smoke on `e2e-test/` dev read-only: doctor / doctor --output json / roles list / status --auth / verify all ok. Concern ruled Critical: a subscription Owner read as missing the control-plane operator roles. Ruling: operator edges are satisfied by the caller's effective permissions at scope covering the role definition's actions and dataActions (7fd1966; Owner covers Search Service Contributor and Foundry Account Owner, does not cover Azure AI/Foundry User or Search Index Data Reader). `Principal::Named` uses that path only when it is the caller's own oid.
- Final review (fable): no Critical. One fix wave (24c75ef, 5fce91e, 169c195): I-1 Foundry Project Manager has dataActions so Owner does not cover it — the Connection operator edge gets Cognitive Services Contributor as an alternative role and the test fake mirrors Azure's real definition; I-2 printed `az role assignment create` uses `--role <guid>` (Microsoft's guidance during the Foundry role rename) and the display names are the current Foundry names; I-3 `auth doctor --fix`, `auth roles remove`, `auth easy-auth` sit behind the protected gate with `--confirm-env`; I-4 `SearchIdentity` check consults the graph (EnableSystemIdentity / AttachUserAssignedIdentity fixes); I-5 `encryptionKey.identity` is a registry infra ref and the CMK edge's principal; M-2 Easy Auth check accepts clientId == appId and reports `Return401`; M-5 word-boundary markers and Ok never downgraded (`Summary.live` keeps exit 4); S3-HD SKU rejection; §6 push gate offers `key-vault`; §11 Foundry-project wiremock scenario; spec §3.2/§3.3/§7 annotated with the 2.0.0 gaps. Re-review clean; controller fixed three minors (c6c6cda).
- Struck by ruling: §3.3 Deny + UAMI "rewrite to system identity" question (reported with a hint; `auth.identity.<env>` reserved); PNA Disabled shared-private-link automation (report only); §9 `error.rs` suggestion text kept; optional temp storage account smoke (network check has wiremock coverage).

Deferred (backlog): M-1 plan-scoped graphs consult the env tree for absent connections; M-3 `--dry-run` writes the bindings cache; M-7 `idtyp` for operator principal type; interactive push may ask the protected question twice when the inline doctor runs first; `RIGG_RBAC_*` shared by two loops; Key Vault secret versions; `restore_key_carriers` index-based; rigg-client env-auth unit tests flake under overlapping `cargo test` runs; doctor omits verify-only roles (only `push --verify`'s preflight reports them); §10 "green second run" test.
