...
**environments** (dev, test, prod) and, per environment, the
**targets, dependencies and policy** that say which Azure AI Search service
and Microsoft Foundry account/project it points at, which infrastructure its
files may reference, and how carefully rigg must treat mutations against it. It also
holds shared assets like `apis/`. A workspace holds *no* resource
definitions itself.
...
skillsets, knowledge bases, agents, and model deployments live as files
inside a project.

**A resource belongs to exactly one project.** rigg enforces this. It is
what makes sync unambiguous: when you push a project, rigg knows exactly
which remote resources that project owns, so it never half-syncs or fights
another project over the same resource.
...
**Shared resources.** Because a resource lives in exactly one project, a
...
The same applies to sub-resources Azure creates automatically, for example
the index and indexer behind a managed-ingestion knowledge source. Manage
the knowledge source; Azure manages what it generates.
...
other pieces of Azure infrastructure its resources may reference, and how
carefully rigg must treat mutations against it.
...
path, has `"name": "docs-index"`.

rigg correlates the two files by path, not by name, so renaming a resource
in one environment never breaks its link to the same resource in another.
...
encryption key, a function app in a custom skill's URL.
...
resolved id is cached in `.rigg/<env>/bindings.json` (gitignored).

**Binding names correlate across environments** exactly as file paths
correlate resources: the same name in `dev` and `prod` is the same *role*,
possibly played by a different physical resource.

**Shared** is the same physical resource bound in two environments.
The binding names may differ, and an `ai-services` binding counts as sharing
another environment's implicit `foundry` target when both name the same
account. Sharing is explicit, never inferred. **Different** means the values
differ, as `docs-storage` does above.
...
environment. Per binding in the model environment, it asks whether the new
...
the command line answer the same questions non-interactively.

`rigg describe` also prints an infrastructure section per environment.
...
|---|---|---|
| Bound | matches a binding of this environment | ok |
| Shared | bound here and in another environment, same value | ok |
| Leak | bound in **another** environment, not this one | **error** |
| Unbound | matches no binding anywhere | warning |
| External | an `api`-form reference with no `api` binding | warning |

**Bound** also covers the implicit `search`/`foundry` bindings. **Shared**
references are listed with `--show-bindings`. A **Leak** is always an error:
the file points at another environment's infrastructure. **Unbound** and
**External** become errors when `policy.strict-bindings: true`.
...
2. **Sibling translation** — every reference to another resource by physical
   ...
   connection, `x-rigg-ref`, and the knowledge-base name inside a
...
`--yes` applies a plan that has no pending questions.
...
`--dry-run`. It covers the search/Foundry targets, a rewiring table per
binding (shared vs. changed, with reference counts), renamed siblings with
the number of references rewritten, a resource summary (changed / new /
...
order: `rigg validate <project>`, `rigg auth doctor -e <to>`,
`rigg push <project> -e <to> --dry-run`, `rigg push <project> -e <to>`.

`rigg diff --compare-env` remains a *raw* environment-vs-environment
comparison. "Equal modulo infrastructure" is what `promote --dry-run` shows,
so there is no separate compare mode for it.
...
`cognitiveServices.identity` field. **`foundry-project`** is what an agent
uses to reach a knowledge base.
...
[below](#the-trusted-services-caveat)), and it needs no binding.

Use a user-assigned identity when you want role assignments to survive
re-creating the service, or one identity shared across environments: bind it
...
`skillset`; other kinds get their identities from `pull` or
`rigg env bind --learn`, not from a fresh scaffold).
...
| Skillset `knowledgeStore.storageConnectionString` | the knowledge store's identity | Storage Blob Data Contributor |
| An embedding `resourceUri` | that element's `authIdentity`, else system | Cognitive Services OpenAI User |
| A chat-completion `resourceUri` | same | Cognitive Services User |
| Skillset `cognitiveServices` with an `AIServicesByIdentity` `subdomainUrl` | its `identity`, else system | Cognitive Services User |
| Skillset WebApiSkill with `authResourceId` | that skill's `authIdentity`, else system | *not a role* |
| Agent `tools[].project_connection_id` using `ProjectManagedIdentity` | `foundry-project` | Search Index Data Reader, on the search service |
...
**The checks, alongside them:**

- the Search SKU (Free has no managed identity; knowledge bases need Basic
  or higher), and whether the service has an identity at all;
- whether the service accepts Entra tokens (`authOptions.aadOrApiKey` or
  `disableLocalAuth`);
...
Model deployment availability and quota are checked by `push`/`promote`,
where the deployment is actually written. Doctor lists the row as skipped
rather than repeating a check whose answer only matters at write time.

**The operator's own edges** come from the plan rather than from a single
field:

- Search Service Contributor on the search service, for any Search resource;
- Foundry User on the Foundry *project*, for agents;
- Foundry Project Manager (or Cognitive Services Contributor) on the
  account, for connections;
- Foundry Account Owner (or Cognitive Services Contributor), for deployments
  and guardrails;
- plus, for every edge rigg might have to grant, whether you can create a
  role assignment at that scope at all.

A push that will also read the data plane (`push --verify`) adds Search
Index Data Reader to its own preflight.

**How an operator edge is satisfied:** by an assignment of that exact role,
by an assignment of one of the alternatives the edge lists, or by your
*effective* permissions at the scope covering everything one of those role
definitions grants.
...
role: ARM will only report effective permissions for the caller.)
...
It exits **0** when everything is in place, **4** when anything is missing
or could not be judged, and **6** when `--fix` needs a confirmation it
cannot ask for (a script, or `--output json`). `--output json` prints
...
`--skip-auth-preflight` opts out entirely, for a caller who knows the wiring
is fine and cannot read ARM.

**`rigg verify <project>`** (also `rigg push --verify`) is the proof a green
...
**What it does:** registers (or reuses) an Entra application with
`api://<app-id>` and a `Caller` app role, creates the enterprise
application, and PUTs a **merged** `authsettingsV2` on the function app.
Other identity providers and unrelated settings are kept; `allowedAudiences`
and `allowedApplications` are unioned, never replaced.

**Who it admits:** the search service's system-assigned identity, or the
...
`rigg push --refresh-credentials` re-injects for skillsets that are
otherwise in sync.
...
**Trusted services means system-assigned.** The exception works only with the
search service's system-assigned identity; a user-assigned identity cannot use
it.

rigg's doctor knows this. A user-assigned identity against firewalled
...
**When rigg needs an answer.** Guided flows ask questions. On a terminal
rigg prompts. In scripts and from AI agents rigg cannot prompt, so it prints
a `needs-input` JSON document listing the questions (id, prompt, candidates)
and exits 6.

Re-run with `--answer <id>=<value>` (repeatable) or `--answers-file <path>`;
answered questions are never asked again. A protected environment's typed
confirmation is such a question (`confirm.protected.<env>`);
`--confirm-env <env>` remains as shorthand.

**What counts as non-interactive:** stdin or stdout is not a terminal,
`--non-interactive`, `--yes` or `--output json`, or `RIGG_NON_INTERACTIVE=1`
is set. The environment variable is the way to force script behaviour while
